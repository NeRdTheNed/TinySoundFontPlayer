...
def main():
  ...
  if not zip:
    ...
    if demo:
      installer = "\\build-win\\installer\\TinySoundFontPlayer Demo Installer.exe"
    
    files = [
      projectpath + installer,
      projectpath + "\\installer\\changelog.txt",
      projectpath + "\\installer\\known-issues.txt",
      projectpath + "\\manual\\TinySoundFontPlayer manual.pdf" 
    ]
  else:
    files = [
  ...
  files = [
    projectpath + "\\build-win\\pdbs\\TinySoundFontPlayer-vst3_x64.pdb",
    projectpath + "\\build-win\\pdbs\\TinySoundFontPlayer-app_x64.pdb",
    projectpath + "\\build-win\\pdbs\\TinySoundFontPlayer-clap_x64.pdb"
  ]
