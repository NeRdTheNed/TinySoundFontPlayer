...
if %DEMO_ARG% == "demo" (
  echo Making TinySoundFontPlayer Windows DEMO VERSION distribution ...
  set DEMO=1
) else (
  echo Making TinySoundFontPlayer Windows FULL VERSION distribution ...
  set DEMO=0
)
...
REM msbuild TinySoundFontPlayer.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 
...
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TinySoundFontPlayer.aaxplugin\Contents\Win32\TinySoundFontPlayer.aaxplugin --out .\build-win\aax\bin\TinySoundFontPlayer.aaxplugin\Contents\Win32\TinySoundFontPlayer.aaxplugin
REM --wraptool sign --verbose --account XXXXX --wcguid XXXXX --keyfile XXXXX.p12 --keypassword XXXXX --in .\build-win\aax\bin\TinySoundFontPlayer.aaxplugin\Contents\x64\TinySoundFontPlayer.aaxplugin --out .\build-win\aax\bin\TinySoundFontPlayer.aaxplugin\Contents\x64\TinySoundFontPlayer.aaxplugin
...
echo Making Installer ...
  ...
  REM :32-Bit-is
  REM REM "%ProgramFiles%\Inno Setup 6\iscc" /Q ".\installer\TinySoundFontPlayer.iss"
  REM goto END-is

  REM :64-Bit-is
  "%ProgramFiles(x86)%\Inno Setup 6\iscc" /Q ".\installer\TinySoundFontPlayer.iss"
  REM goto END-is

  REM :END-is

  REM - Codesign Installer for Windows 8+
  REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "TinySoundFontPlayer Installer" ".\installer\TinySoundFontPlayer Installer.exe"

  REM -if %1 == 1 (
  REM -copy ".\installer\TinySoundFontPlayer Installer.exe" ".\installer\TinySoundFontPlayer Demo Installer.exe"
  REM -del ".\installer\TinySoundFontPlayer Installer.exe"
  REM -)
