...
: iplug::Plugin(info, MakeConfig(kNumParams, kNumPresets))
{
  ...
  mLayoutFunc = [&](IGraphics* pGraphics) {
    ...
    pGraphics->AttachControl(new ITextControl(titleBounds, "TinySoundFontPlayer", IText(30)), kCtrlTagTitle);
...
#if IPLUG_EDITOR
void TinySoundFontPlayer::OnParentWindowResize(int width, int height)
{
  if(GetUI())
